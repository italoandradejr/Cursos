...
botaoAdicionar.addEventListener("click", function (event) {
    ...
    form.reset()
    var mensagemnsErro = document.querySelector("#msg-erro")
    mensagemnsErro.innerHTML = ""
})
...
function validaPaciente(paciente) {
    ...
    if (paciente.gordura.length == 0) {
        erros.push("Preencha a gordura corretamente")
    }

    if (paciente.peso.length == 0) {
        erros.push("Preencha o peso corretamente")
    }

    if (paciente.altura.length == 0) {
        erros.push("Preencha a altura corretamente")
    }
    return erros
}